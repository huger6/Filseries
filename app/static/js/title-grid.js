/**
 * Title Grid - Shared JavaScript Module
 * Handles infinite scroll, filtering, sorting, and carousels
 * 
 * Usage:
 *   initTitleGridPage({
 *       gridId: 'titleGrid',
 *       filterBarSelector: '.title-filter-bar',
 *       moviesUrl: '/watched/movies',
 *       seriesUrl: '/watched/series',
 *       similarUrl: '/watched/similar',
 *       recommendationsUrl: '/watched/recommendations',
 *       titleBaseUrl: '/title',
 *       defaultPosterUrl: '/static/img/default-poster.png',
 *       cardClass: 'title-card',
 *       errorMessage: 'Failed to load your titles. Please try again.'
 *   });
 */

function initTitleGridPage(config) {
    ...
    async function fetchTitles(mediaType, cursor = null) {
        const endpoint = mediaType === 'movie' ? config.moviesUrl : config.seriesUrl;
    ...
    function createTitleCard(title) {
        ...
        const cardClass = config.cardClass || 'title-card';
        
        return `
            <div class="${cardClass}" data-id="${title.id}" data-type="${title.media_type}">
                <a href="${config.titleBaseUrl}/${title.media_type}/${title.id}" class="card-link">
    ...
    function showError() {
        const errorMessage = config.errorMessage || 'Failed to load your titles. Please try again.';
        if (grid) {
            grid.innerHTML = `
    ...
    async function loadSimilarTitles() {
        ...
        try {
            const response = await fetch(config.similarUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    async function loadRecommendations() {
        ...
        try {
            const response = await fetch(config.recommendationsUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    function renderCarouselCards(track, titles) {
        const html = titles.map(title => {
            ...
            return `
                <a href="${config.titleBaseUrl}/${mediaType}/${title.id}" class="carousel-card">
                    <div class="card-poster">
                        <img src="${posterUrl}" alt="${escapeHtml(title.title)}" loading="lazy" class="poster-img${!title.poster_path ? ' default-poster' : ''}">
